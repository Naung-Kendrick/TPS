-- ═══════════════════════════════════════════════════════════════════════════
-- TPS — Add 6 functional age group counts to stats_breakdown RPC
--       (Age Pyramid / လူမှုဘဝကဏ္ဍအလိုက် ခွဲခြားခြင်း)
--
-- HOW TO INSTALL:
...
-- ─────────────────────────────────────────────────────────────────────────────
-- Helper: age_bucket_6(text) → 6-group classification
--   '0_4'   → 0–4    ကလေးငယ်များ (Infants / Toddlers)
--   '5_13'  → 5–13   ကျောင်းနေအရွယ် (School-age Children)
--   '14_17' → 14–17  အထက်တန်းအရွယ်လူငယ် (Adolescents)
--   '18_25' → 18–25  လူငယ်/အလုပ်စဝင်မည့်အရွယ် (Young Adults)
--   '26_59' → 26–59  အဓိကလုပ်သားအင်အား (Prime Working Age)
--   '60p'   → 60+    သက်ကြီးရွယ်အိုများ (Elderly / Seniors)
--   'unknown'
-- ─────────────────────────────────────────────────────────────────────────────
CREATE OR REPLACE FUNCTION public.age_bucket_6(d text)
RETURNS text
LANGUAGE sql IMMUTABLE
AS $$
  WITH x AS (
    SELECT COALESCE(EXTRACT(year FROM age(public.parse_myanmar_date(d)))::int, -1) AS yr
  )
  SELECT CASE
    WHEN yr < 0    THEN 'unknown'
    WHEN yr <= 4   THEN '0_4'
    WHEN yr <= 13  THEN '5_13'
    WHEN yr <= 17  THEN '14_17'
    WHEN yr <= 25  THEN '18_25'
    WHEN yr <= 59  THEN '26_59'
    ELSE '60p'
  END
  FROM x;
$$;
...
-- ─────────────────────────────────────────────────────────────────────────────
-- Replace stats_breakdown — adds ag* fields to totalStats
-- ─────────────────────────────────────────────────────────────────────────────
...
AS $$
  ...
  base AS (
    SELECT
      ...
      occupation, taang_land_id_no, date_of_birth,
      district, township, ward_village_group,
      public.age_bucket(date_of_birth)   AS age_b,
      public.age_bucket_6(date_of_birth) AS age_b6
  ...
  unique_people AS (
    SELECT DISTINCT ON (id)
      id, household_no, gender, religious, nationality, resident_status,
      occupation, taang_land_id_no, age_b, age_b6
    FROM expanded
  ),
  rel_total AS (
  ...
  total_stats AS (
    SELECT jsonb_build_object(
      ...
      -- ── 6 functional age groups ─────────────────────────────────────────
      'ag0_4m',      (SELECT COUNT(*) FROM unique_people WHERE age_b6='0_4'   AND gender IN ('ကျား','က')),
      'ag0_4f',      (SELECT COUNT(*) FROM unique_people WHERE age_b6='0_4'   AND gender='မ'),
      'ag5_13m',     (SELECT COUNT(*) FROM unique_people WHERE age_b6='5_13'  AND gender IN ('ကျား','က')),
      'ag5_13f',     (SELECT COUNT(*) FROM unique_people WHERE age_b6='5_13'  AND gender='မ'),
      'ag14_17m',    (SELECT COUNT(*) FROM unique_people WHERE age_b6='14_17' AND gender IN ('ကျား','က')),
      'ag14_17f',    (SELECT COUNT(*) FROM unique_people WHERE age_b6='14_17' AND gender='မ'),
      'ag18_25m',    (SELECT COUNT(*) FROM unique_people WHERE age_b6='18_25' AND gender IN ('ကျား','က')),
      'ag18_25f',    (SELECT COUNT(*) FROM unique_people WHERE age_b6='18_25' AND gender='မ'),
      'ag26_59m',    (SELECT COUNT(*) FROM unique_people WHERE age_b6='26_59' AND gender IN ('ကျား','က')),
      'ag26_59f',    (SELECT COUNT(*) FROM unique_people WHERE age_b6='26_59' AND gender='မ'),
      'ag60pm',      (SELECT COUNT(*) FROM unique_people WHERE age_b6='60p'   AND gender IN ('ကျား','က')),
      'ag60pf',      (SELECT COUNT(*) FROM unique_people WHERE age_b6='60p'   AND gender='မ')
    ) AS v_total
  ),
  per_group AS (
    SELECT
...
GRANT EXECUTE ON FUNCTION public.age_bucket_6(text) TO anon, authenticated;
